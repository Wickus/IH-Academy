...
import { Trash2, Plus, Users, Crown, UserCheck, UserMinus } from "lucide-react";
import { useToast } from "@/hooks/use-toast";
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogTrigger } from "@/components/ui/dialog";
import { apiRequest } from "@/lib/queryClient";

interface ClassCoachesFormProps {
  classId: number;
  organization: any;
}

export default function ClassCoachesForm({ classId, organization }: ClassCoachesFormProps) {
  ...
  // Fetch assigned coaches for this class
  const { data: classCoaches = [] } = useQuery({
    queryKey: ['/api/classes', classId, 'coaches'],
    queryFn: () => apiRequest('GET', `/api/classes/${classId}/coaches`),
    enabled: !!classId,
  });
  ...
  const assignCoachMutation = useMutation({
    mutationFn: ({ coachId, role }: { coachId: number; role: string }) =>
      apiRequest('POST', `/api/classes/${classId}/coaches`, { coachId, role }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/classes', classId, 'coaches'] });
      setOpen(false);
      setSelectedCoachId(null);
      toast({
        title: "Coach Assigned",
        description: "Coach has been successfully assigned to this class.",
      });
    },
    onError: (error: any) => {
      toast({
        title: "Assignment Failed",
        description: error.message || "Failed to assign coach to class.",
        variant: "destructive",
      });
    },
  });
  ...
  const removeCoachMutation = useMutation({
    mutationFn: (coachId: number) =>
      apiRequest('DELETE', `/api/classes/${classId}/coaches/${coachId}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/classes', classId, 'coaches'] });
      toast({
        title: "Coach Removed",
        description: "Coach has been removed from this class.",
      });
    },
    onError: (error: any) => {
      toast({
        title: "Removal Failed",
        description: error.message || "Failed to remove coach from class.",
        variant: "destructive",
      });
    },
  });
  ...
  const updateRoleMutation = useMutation({
    mutationFn: ({ coachId, role }: { coachId: number; role: string }) =>
      apiRequest('PUT', `/api/classes/${classId}/coaches/${coachId}`, { role }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/classes', classId, 'coaches'] });
      toast({
        title: "Role Updated",
      ...
      <CardContent className="space-y-4">
        ...
        {availableCoaches.length > 0 && (
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Assign Coach to Class</DialogTitle>
              </DialogHeader>
